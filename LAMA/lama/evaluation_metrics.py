...
def __max_probs_values_indices(masked_indices, log_probs, topk=100):
    ...
    for i in range(0, len(masked_indices)):
        ...
        log_prob = log_probs[masked_index]

        value_max_probs, index_max_probs = torch.topk(input=log_prob, k=topk, dim=0)
        index_max_probs = index_max_probs.numpy().astype(int)
        value_max_probs = value_max_probs.detach().numpy()

        result.append([log_probs, index_max_probs, value_max_probs])
    return result
...
def get_ranking(log_probs, masked_indices, vocab, label_index=None, index_list=None, topk=100, P_AT=10,
    ...
    for result in __max_probs_values_indices(masked_indices, log_probs, topk=topk):
        experiment_result = {}
        log_probs, index_max_probs, value_max_probs = result
        result_masked_topk = __print_top_k(value_max_probs, index_max_probs, vocab, topk, index_list)
